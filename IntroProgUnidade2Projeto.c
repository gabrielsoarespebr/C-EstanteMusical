    ...
    while (estante.menu != TELA_SAIR)
	{
    	switch(estante.menu)
		{
			...
			case TELA_EXCLUIR:
			{
				...
				do
				{
					printf("Indique a posicao da musica a ser excluida (de zero em diante): ");
					scanf("%d", &excluido);
				} while (excluido < 0 || excluido >= estante.totalMusicas);
				
				for (ordem = excluido; ordem <= estante.totalMusicas; ordem++)
				{
					estante.musicas[ordem] = estante.musicas[ordem++];
					estante.totalMusicas--;
				}
				estante.menu = TELA_PRINCIPAL;
				break;
